...
use crate::{
    ...
    util::{address_to_pubkey, decode_hex, expand_path},
};
use mosaic::state::{
    PackUnpack,
    root::Root,
...
pub(crate) fn load_session_spec(path: &Path) -> Result<LoadedSessionSpec> {
    ...
    let instruction_data = decode_hex(&spec.data)
        .with_context(|| format!("invalid hex instruction data in {}", path.display()))?;
...
pub(crate) fn load_inline_session_spec(
    ...
    let instruction_data =
        decode_hex(data).context("invalid hex instruction data passed to --data")?;
