...
pub(crate) fn close_session_command(
    ...
    let instruction = build_close_session_instruction(config, &payer.pubkey(), &resolved)?;
    let signature = send_instruction(rpc, config, &payer, instruction, &[])?;

    print_json(&SessionActionResultView {
        action: "close-session".to_owned(),
        signature,
